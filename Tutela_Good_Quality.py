...
df_tutela = df_tutela[['location','region','operator','good_quality','yearweek']]

df_tutela.loc[df_tutela['operator']=="Telkomsel", 'Telkomsel'] = df_tutela['good_quality']
df_tutela.loc[df_tutela['operator']=="Telkomsel", 'Telkomsel_null'] = df_tutela['good_quality']
df_tutela.loc[df_tutela['operator']=="XL", 'XL'] = df_tutela['good_quality']
df_tutela.loc[df_tutela['operator']=="XL", 'XL_null'] = df_tutela['good_quality']
df_tutela.loc[df_tutela['operator']=="Indosat Ooredoo + 3", 'Indosat Ooredoo + 3'] = df_tutela['good_quality']
df_tutela.loc[df_tutela['operator']=="Indosat Ooredoo + 3", 'Indosat Ooredoo + 3_null'] = df_tutela['good_quality']
df_tutela.loc[df_tutela['operator']=="Smartfren", 'Smartfren'] = df_tutela['good_quality']
df_tutela.loc[df_tutela['operator']=="Smartfren", 'Smartfren_null'] = df_tutela['good_quality']
...
df_tutela["Category"] = "Good Quality"
...
print(df_final)
path_to_csv = "path/tutela_good_quality.csv"
df_final.fillna('\\N', inplace=True)
df_final.to_csv(path_to_csv, index=None)


cursor = conn.cursor()
sql_2 = "LOAD DATA LOCAL INFILE 'path/tutela_good_quality.csv' INTO TABLE tutela_border_mm_kabupaten_nation_ci FIELDS TERMINATED BY ',' ENCLOSED BY '\"' IGNORE 1 LINES"
cursor.execute(sql_2)
conn.commit()
cursor.close()
print(cursor.rowcount, "rows was inserted")
print("Upload Done")
print("Run Tutela Netflix")
tutela_Video_Score_Netflix = 'path/Tutela_Video_Score_Netflix.py'
exec(open(tutela_Video_Score_Netflix).read())